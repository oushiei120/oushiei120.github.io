...
def convert_to_iiif(input_json_path, base_url):
    ...
    for idx, para in enumerate(source_data.get("paragraphs", [])):
        if "contents" in para and "box" in para:
            ...
            # 创建单个注释
            annotation = {
                "id": f"{base_url}/iiif/annotation/p1-{idx+1}",
    ...
    iiif_data = {
        "@context": "http://iiif.io/api/presentation/3/context.json",
        "id": f"{base_url}/iiif/annotations.json",
        "type": "AnnotationPage",
        "items": annotations
    }
    
    # 生成输出文件路径
    output_path = "/Users/oushiei/Documents/GitHub/oushiei120.github.io/iiif/annotations.json"
